...
/**
 * The main components of the events edit page
 *
 * @returns The JSX element
 */
function Components(): JSX.Element {
  ...
  const { mutateAsync: getEvent, status: getEventStatus } =
    trpc.getEvent.useMutation();
  const { mutateAsync: updateEvent, status: updateEventStatus } =
    trpc.updateEvent.useMutation();

  const [image, setImage] = useState<File | null>(null);
  const [event, setEvent] = useState<ClubEvent | null>(null);

  useEffect(() => {
    if (getEventStatus === "loading" || getEventStatus === "success") {
      return;
    }
  ...
  if (!event) {
    return (
      <MainWrapper className="relative z-10 w-full justify-start gap-2 px-12 py-40 text-center">
        ...
        <p className="my-4 text-gray-400">
          The event you are trying to access does not exist.
        </p>
        <Button href="/events" as={Link}>
          View all events
        </Button>
      </MainWrapper>
    );
  ...
  if (!hasPermissions(session.user.permissions, [Permission.EDIT_EVENT])) {
    return (
      <MainWrapper className="relative z-10 w-full justify-start gap-2 px-12 py-40 text-center">
        <h1 className="text-7xl font-extrabold tracking-wide text-foreground">
          Unauthorized
        ...
        <p className="my-4 text-gray-400">
          You do not have permission to access this page. (need: edit_event)
        </p>
        <Button href="/auth/signin" as={Link}>
          Switch accounts
        </Button>
